=head1 NAME
 ...
 iMSCP::Installer::Adapter::Debian - Debian installer adapter
...
package iMSCP::Installer::Adapter::Debian;
...
use File::Temp;
use version;
use parent 'iMSCP::Installer::Adapter::Abstract';

=head1 DESCRIPTION

 i-MSCP installer adapter implementation for Debian.
...
sub installPreRequiredPackages
{
    ...
    my $cmd = 'apt-get';
    iMSCP::ProgramFinder::find( $cmd ) or die( 'apt-get command not found' );
...
sub installPackages
{
    ...
    my $nbTasks = scalar @{$self->{'packagesPreinst'}};
    my $cTask = 1;
    startDetail();
    for my $task(@{$self->{'packagesPreinst'}}) {
        $rs ||= step(
            sub {
                my $stdout;
                $rs = execute(
                    ((index($task->[1], '/') == 0)
                        ? '' : "/bin/sh $FindBin::Bin/installer/preinstall "
                    ).$task->[1],
            ...
            sprintf( "Executing `%s' package preinstall tasks...", $task->[0], ), $nbTasks, $cTask
        );
        last if $rs;
        $cTask++;
    }
    undef $self->{'packagesPreinst'};
    endDetail();
    return $rs if $rs;
    ...
    $nbTasks = scalar @{$self->{'packagesPostinst'}};
    $cTask = 1;
    startDetail();
    for my $task(@{$self->{'packagesPostinst'}}) {
        $rs ||= step(
            sub {
                my $stdout;
                $rs = execute(
                    ((index($task->[1], '/') == 0)
                        ? '' : "/bin/sh $FindBin::Bin/installer/postinstall "
                    ).$task->[1],
                    (iMSCP::Getopt->noprompt && iMSCP::Getopt->verbose ? undef : \ $stdout),
                    \ my $stderr
                );
                error($stderr || sprintf("Unknown error", $task)) if $rs;
                $rs;
            },
            sprintf( "Executing `%s' package postinstall tasks...", $task->[0] ), $nbTasks, $cTask
        );
        last if $rs;
        $cTask++;
    }
    undef $self->{'packagesPostinst'};
    endDetail();
    return $rs if $rs;
    ...
    $rs ||= iMSCP::EventManager->getInstance()->trigger( 'afterInstallPackages' );
}
...
sub uninstallPackages
{
    ...
    my $rs = iMSCP::EventManager->getInstance()->trigger( 'beforeUninstallPackages', $packagesToUninstall );
    return $rs if $rs;
...
=item _init()
 ...
 Return iMSCP::Installer::Adapter::Debian
...
sub _init
{
    ...
    $self->{'packagesPreinst'} = [ ];
    $self->{'packagesPostinst'} = [ ];
...
sub _buildPackageList
{
    ...
    my $rs = iMSCP::EventManager->getInstance()->trigger( 'onBuildPackageList', \ my $packageFilePath );
    return $rs if $rs;
    ...
    unless (defined $packageFilePath) {
        ...
        $packageFilePath = "$FindBin::Bin/installer/Packages/".lc($distroID).'-'.lc($distroCodename).'.xml';
    }
        ...
        if ($data->{$sAlt}->{'package'}) {
            for(@{$data->{$sAlt}->{'package'}}) {
                if (ref $_ eq 'HASH') {
                    ...
                    push @{$self->{'packagesPostinst'}}, [ $_->{'content'}, $_->{'postinst'} ] if $_->{'postinst'};
                    push @{$self->{'packagesPreinst'}}, [ $_->{'content'}, $_->{'preinst'} ] if $_->{'preinst'};
                } else {
    ...
    for my $sec(@{$self->{'repositorySections'}}) {
        ...
        while($fileContent =~ /^deb\s+(?<uri>(?:https?|ftp)[^\s]+)\s+(?<distrib>[^\s]+)\s+(?<components>.+)$/gm) {
            ...
            if ($fileContent !~ /^deb\s+$rc{'uri'}\s+\b$rc{'distrib'}\b\s+.*\b$sec\b/m) {
                my $rs = execute(
                    ['wget', '--spider', "$rc{'uri'}/dists/$rc{'distrib'}/$sec/" ], \ my $stdout, \ my $stderr
                );
                debug($stdout) if $stdout;
...
EOF
        ...
        if ($repository->{'repository_key_srv'}) { # Add the repository key from the given key server
            ...
            @cmd = ('wget --prefer-family=IPv4 -qO-', escapeShell( $repository->{'repository_key_uri'} ),
                '| apt-key add -');
        }
...
sub _rebuildAndInstallPackage
{
    ...
    my $lsbRelease = iMSCP::LsbRelease->getInstance();
    $patchesDir = "$FindBin::Bin/config/".lc( $lsbRelease->getId( 1 ) )."/$patchesDir";
    unless (-d $patchesDir) {
        ...
        sub {
            if ($self->{'need_pbuilder_update'}) {
                ...
                my $cmd = [
                    ...
                    '--configfile', "$FindBin::Bin/config/".lc( $lsbRelease->getId( 1 ) ).'/pbuilder/pbuilderrc',
                    '--override-config'
                ];
                my $stderr;
                $rs = executeNoWait(
