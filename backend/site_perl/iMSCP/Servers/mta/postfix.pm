=head1 NAME
 ...
 iMSCP::Servers::mta::postfix - i-MSCP Postfix MTA server implementation
...
package iMSCP::Servers::mta::postfix;

use strict;
use warnings;
use Class::Autouse qw/ :nostat File::Temp iMSCP::Servers::mta::postfix::installer iMSCP::Servers::mta::postfix::uninstaller /;
use File::Basename;
use iMSCP::Config;
use iMSCP::Debug;
use iMSCP::Dir;
...
sub preinstall
{
    ...
    $rs ||= $self->stop();
    $rs ||= $rs = iMSCP::Servers::mta::postfix::installer->getInstance()->preinstall();
...
sub install
{
    ...
    $rs ||= iMSCP::Servers::mta::postfix::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Servers::mta::postfix::uninstaller->getInstance()->uninstall();
    $rs ||= $self->restart();
...
=item postconf($conffile, %params)
 ...
 Param hash %params A hash where each key is a Postfix parameter name and associated value, a hashes describing in order:
  ...
  iMSCP::Servers::mta::postfix->getInstance(
    (
...
=item _init()
 ...
 Return iMSCP::Servers::mta::postfix
