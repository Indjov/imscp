=head1 NAME
 ...
 iMSCP::Servers::ftpd::vsftpd - i-MSCP VsFTPd Server implementation
...
package iMSCP::Servers::ftpd::vsftpd;
...
use Class::Autouse qw/ :nostat iMSCP::Servers::ftpd::vsftpd::installer iMSCP::Servers::ftpd::vsftpd::uninstaller /;
...
sub registerSetupListeners
{
    ...
    iMSCP::Servers::ftpd::vsftpd::installer->getInstance()->registerSetupListeners( $eventManager );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Servers::ftpd::vsftpd::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Servers::ftpd::vsftpd::uninstaller->getInstance()->uninstall();
    $rs || ($self->{'restart'} = 1);
...
=item addUser(\%data)
 ...
 Param hash \%data user data as provided by iMSCP::Modules::FtpUser module
 Return int 0 on success, other on failure
...
=item addFtpUser(\%data)
 ...
 Param hash \%data Ftp user as provided by iMSCP::Modules::FtpUser module
 Return int 0 on success, other on failure
...
=item _init()
 ...
 Return iMSCP::Servers::ftpd::vsftpd
