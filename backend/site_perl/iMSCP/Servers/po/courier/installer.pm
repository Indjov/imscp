=head1 NAME
 ...
 iMSCP::Servers::po::courier::installer - i-MSCP Courier IMAP/POP3 Server installer implementation
...
package iMSCP::Servers::po::courier::installer;
...
use iMSCP::Umask;
use iMSCP::Servers::mta::postfix;
use iMSCP::Servers::po::courier;
...
Please enter a username for the Courier Authdaemon SQL user:$msg
EOF
            $msg = '';
...
=item _init()
 ...
 Return iMSCP::Servers::po::courier::installer
...
sub _init
{
    ...
    $self->{'po'} = iMSCP::Servers::po::courier->getInstance();
    $self->{'mta'} = iMSCP::Servers::mta::postfix->getInstance();
...
sub _buildConf
{
    ...
    if (-f "$self->{'cfgDir'}/imapd.local") {
        ...
        $fileContent = replaceBloc(
            "\n# iMSCP::Servers::po::courier::installer - BEGIN\n",
            "# iMSCP::Servers::po::courier::installer - ENDING\n",
            '',
            $fileContent
        );
...
sub _setupCyrusSASL
{
    ...
    my $data = {
        SASL_AUTH_SOCKET_PATH => $self->{'config'}->{'SASL_AUTH_SOCKET_PATH'},
        SASL_LOG_LEVEL        => $self->{'config'}->{'SASL_LOG_LEVEL'}
    };
    ...
    $rs = $self->{'eventManager'}->trigger( 'afterPoBuildSaslConfFile', \ $cfgTpl, 'smtpd.conf' );
    return $rs if $rs;
