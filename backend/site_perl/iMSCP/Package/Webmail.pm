...
iMSCP::Package::Webmail - i-MSCP Webmail package
...
EOF
    ...
    for (keys %{$self->{'PACKAGES'}}) {
        ...
        my $package = "iMSCP::Package::Webmail::${_}::${_}";
...
sub _init
{
    ...
    @{$self->{'PACKAGES'}}{
        iMSCP::Dir->new( dirname => "$main::imscpConfig{'ENGINE_LIB_DIR'}/Package/Webmail" )->getDirs()
    } = ();
