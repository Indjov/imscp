...
iMSCP::Package::PhpMyAdmin::Installer - i-MSCP PhpMyAdmin package installer
...
package iMSCP::Package::PhpMyAdmin::Installer;
...
use iMSCP::Package::FrontEnd;
use iMSCP::Package::PhpMyAdmin;
...
=item _init()
 ...
 Return iMSCP::Package::PhpMyAdmin::Installer
...
sub _init
{
    ...
    $self->{'phpmyadmin'} = iMSCP::Package::PhpMyAdmin->getInstance();
...
{
    my $frontEnd = iMSCP::Package::FrontEnd->getInstance();
    $frontEnd->buildConfFile(
        "$main::imscpConfig{'ENGINE_LIB_DIR'}/Package/PhpMyAdmin/config/nginx/imscp_pma.conf",
        {
