...
iMSCP::Package::AntiRootkits::Rkhunter::Installer - i-MSCP Rkhunter package installer
...
package iMSCP::Package::AntiRootkits::Rkhunter::Installer;
...
sub _addCronTask
{
    iMSCP::Servers::cron->factory()->addTask(
        {
            TASKID  => 'iMSCP::Package::AntiRootkits::Rkhunter',
            MINUTE  => '@weekly',
            HOUR    => '',
            DAY     => '',
            MONTH   => '',
            ...
            COMMAND => 'nice -n 15 ionice -c2 -n5 perl '
                ."$main::imscpConfig{'ENGINE_LIB_DIR'}/Package/AntiRootkits/Rkhunter/Cron.pl "
                ."> /dev/null 2>&1"
        }
    );
}
...
sub _scheduleCheck
{
    ...
    $rs = execute(
        "echo 'perl $main::imscpConfig{'ENGINE_LIB_DIR'}/Package/AntiRootkits/Rkhunter/Cron.pl > /dev/null 2>&1' "
            ."| batch",
        \ my $stdout,
        \ my $stderr
    );
