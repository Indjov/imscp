...
iMSCP::Package::FrontEnd - i-MSCP FrontEnd package
...
package iMSCP::Package::FrontEnd;

use strict;
use warnings;
use Class::Autouse qw/ :nostat iMSCP::Package::FrontEnd::Installer iMSCP::Package::FrontEnd::Uninstaller /;
use File::Basename;
use iMSCP::Config;
use iMSCP::Debug;
use iMSCP::Execute;
...
sub registerSetupListeners
{
    ...
    iMSCP::Package::FrontEnd::Installer->getInstance()->registerSetupListeners( $eventManager );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Package::FrontEnd::Installer->getInstance()->install();
...
sub dpkgPostInvokeTasks
{
    ...
    $rs ||= iMSCP::Package::FrontEnd::Installer->getInstance()->dpkgPostInvokeTasks();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Package::FrontEnd::Uninstaller->getInstance()->uninstall();
...
sub _init
{
    ...
    $self->{'phpConfig'} = iMSCP::Servers::httpd->factory()->{'phpConfig'};
...
END
    {
        ...
        my $self = iMSCP::Package::FrontEnd->getInstance();
        if ($self->{'start'}) {
            $? = $self->start();
        } elsif ($self->{'restart'}) {
            $? = $self->restart();
