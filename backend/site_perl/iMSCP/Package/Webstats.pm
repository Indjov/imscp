...
iMSCP::Package::Webstats - i-MSCP Webstats package
...
EOF
    ...
    for (keys %{$self->{'PACKAGES'}}) {
        ...
        my $package = "iMSCP::Package::Webstats::${_}::${_}";
...
sub _init
{
    ...
    @{$self->{'PACKAGES'}}{
        iMSCP::Dir->new( dirname => "$main::imscpConfig{'ENGINE_LIB_DIR'}/Package/Webstats" )->getDirs()
    } = ();
    $self;
}
