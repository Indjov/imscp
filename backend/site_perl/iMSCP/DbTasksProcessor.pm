...
sub process
{
    ...
    $self->_process(
        'iMSCP::Modules::NetworkInterfaces',
        "
    ...
    if ($ipsModule || $self->{'mode'} eq 'setup') {
        ...
        eval { require iMSCP::Modules::Ips } or die( sprintf( "Couldn't load `iMSCP::Module::Ips' module: %s", $@ ) );
        iMSCP::Modules::Ips->new()->process() == 0 or die(
            sprintf(
    ...
    if (%{$rdata}) {
        ...
        for (values %{$rdata}) {
            ...
            my ($stdout, $stderr);
            execute(['imscp-sw-mngr', $pushString ], \$stdout, \$stderr) == 0 or die( $stderr || 'Unknown error' );
    ...
    if (%{$rdata}) {
        ...
        for (values %{$rdata}) {
            ...
            my ($stdout, $stderr);
            execute([ 'imscp-pkt-mngr', $pushstring ], \$stdout, \$stderr) == 0 or die( $stderr || 'Unknown error' );
            debug( $stdout ) if $stdout;
...
sub _process
{
    ...
    eval "require $module" or die( sprintf( "Couldn't load `%s' module: %s", $module, $@ ) );
