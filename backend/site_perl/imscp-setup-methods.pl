...
{
    my $rs = iMSCP::EventManager->getInstance()->trigger('beforeSetupInstallFiles', $main::{'iMSCP_PREFIX'});
    return $rs if $rs;
    ...
    $rs = iMSCP::Dir->new( dirname => $main::{'iMSCP_PREFIX'} )->rcopy( '/' );
    $rs ||= iMSCP::EventManager->getInstance()->trigger('afterSetupInstallFiles', $main::{'iMSCP_PREFIX'});
}
...
{
    my $rs = iMSCP::EventManager->getInstance()->trigger('beforeSetupDeleteBuildDir', $main::{'iMSCP_PREFIX'});
    $rs ||= iMSCP::Dir->new( dirname => $main::{'iMSCP_PREFIX'} )->remove();
    $rs ||= iMSCP::EventManager->getInstance()->trigger('afterSetupDeleteBuildDir', $main::{'iMSCP_PREFIX'});
}
